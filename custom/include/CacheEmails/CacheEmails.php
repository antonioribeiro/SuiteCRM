...
class CacheEmails{
	function import_emails() {		
		...
		$ieBoxes = array();
		$sql = "SELECT ie_id, mbox, msgno, imap_uid FROM email_cache WHERE deleted='0' AND transfered='0' AND ie_id IN (SELECT id FROM inbound_email WHERE deleted='0' AND status='Active') order by senddate desc";
		$res = $db->query($sql);
		$countLimit = 0;
