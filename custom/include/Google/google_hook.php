...
class GoogleHook
{
    function geventHandler($bean, $event)
    {
        ...
        if ($bean->module_dir == 'Users') {
            $pass_encrypt = blowfishEncode(blowfishGetKey('encrypt_field'), $bean->gmail_pass);
            if (empty($bean->fetched_row) || empty($bean->fetched_row['gmail_pass'])) {
                $bean->gmail_pass = $pass_encrypt;
            } else if (!empty($bean->fetched_row['gmail_pass']) && blowfishEncode(blowfishGetKey('encrypt_field'), $bean->fetched_row['gmail_pass']) != $pass_encrypt) {
                $bean->gmail_pass = $pass_encrypt;
            }
            if (!empty($bean->fetched_row) && $bean->fetched_row['gmail_id'] != $bean->gmail_id) {
                //start clean sync for cal
                if (!empty($bean->fetched_row['gmail_id']) && !empty($bean->fetched_row['gmail_pass'])) {
                    $gh = new GoogleHelper();
                    $gh->cleanSync($bean->fetched_row['gmail_id'], blowfishDecode(blowfishGetKey('encrypt_field'), $bean->fetched_row['gmail_pass']), $bean->id);
                }
    ...
    //END: edit 02-01-2013 for docs
    //Validate license key after login
    function validateLicense($bean, $event)
    {
        global $current_user;
        ...
        if (is_admin($current_user)) {
            if (!OutfittersLicense::validateModule()) {
                SugarApplication::redirect("index.php?module=Schedulers&action=license");
                //2013-11-01J
            } else {
                $admin = new Administration();
                $admin->retrieveSettings();
                if (isset($admin->settings[$outfitters_config['shortname'].'_validated_users'])) {
					if($admin->settings[$outfitters_config['shortname'].'_validated_users'] == true){
					}else{
						SugarApplication::redirect("index.php?module=rt_GSync&action=configureUsers&sync_stopped=1");
					}
                }
            }
        }
