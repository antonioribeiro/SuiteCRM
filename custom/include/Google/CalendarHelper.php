	...
	class CalendarHelper{
		public static function updateFromGoogle($client, $userID,$userEmailID, $lastSync,$recurrence=false,$prefrences=array()){
			...
			if($recurrence===false){
				...
				// TODO
				// $query->setStartMin(date('Y-m-d',strtotime(gmdate("Y-m-d"))." -90 days") . "T00:00:01.000Z"); //include events having start date < (current date-90 days)
				// $query->setStartMax(str_replace(' ', 'T', date('Y-m-d',strtotime(gmdate("Y-m-d")." +31 days")) . "T23:59:59.000Z"));
			...
			$GLOBALS['log']->debug(print_r($event_feeds,1));
		...
		public static function sendUpdatedToGoogle($client,$beans,$userID){
			...
			foreach($beans as $bean){
				...
				if($bean->gevent_id != ''){
					$newEvent = $service->events->get('primary', $bean->gevent_id);
				}
